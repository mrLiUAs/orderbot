...
def handle_message(event):
    ...
    if isinstance(event, PostbackEvent):
        ...
        if backdata.get('action') == '要美乃滋':
            ...
            func.send_back_class(event, backdata)
        ...
        elif event.postback.data.split('&')[0] == 'GRADE':
            try:
                tmp_d[event.source.user_id]['grade'] = event.postback.data.split('&')[1]
            except:
                replyText(event, "抱歉，革命失敗，請再試一次")
        elif event.postback.data.split('&')[0] == 'CLASS':
            try:
                tmp_d[event.source.user_id]['class'] = event.postback.data.split('&')[1]
            except:
                replyText(event, "抱歉，革命失敗，請再試一次")
